...
void ResultDatabase::DumpDetailed(ostream& out) {
    ...
    decltype(sorted[0].value.size()) maxtrials = 1;
    for (decltype(sorted.size()) i = 0; i < sorted.size(); i++) {
    ...
    out << setw(testNameW) << "test\t" << setw(attW) << "atts\t" << setw(fieldW) << "median\t"
        ...
        << "min\t"
        << "max\t";
    for (decltype(maxtrials) i = 0; i < maxtrials; i++) out << "trial" << i << "\t";
    out << endl;
    ...
    for (decltype(sorted.size()) i = 0; i < sorted.size(); i++) {
        ...
        if (r.GetMax() == FLT_MAX)
            ...
            out << r.GetMax() << "\t";
        for (decltype(r.value.size()) j = 0; j < r.value.size(); j++) {
